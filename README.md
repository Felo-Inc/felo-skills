...
- **异步生成**：提交提示后会轮询任务状态，完成后输出 `live_doc_url`，无需本地 Office。
- **使用方式**：
  - **终端**：`felo slides "你的主题或描述"`
  - **Claude Code**：安装 `felo-slides` 技能后，可直接说“生成一份产品介绍 PPT”或手动输入 `/felo-slides 你的主题`。
  - 终端可选 `--verbose` 查看轮询进度，`--json` 输出原始 JSON（含 `task_id`、`live_doc_url`）。
...
Ask Claude: "/felo-slides Felo 产品介绍，3 页"
```

**You're done!** `felo-ai` 会在需要实时信息时自动触发，`felo-slides` 用于生成演示文稿。
...
在 Claude Code 中可按需安装两个技能：

- `felo-ai`：**实时搜索**自动触发。适用于天气、新闻、价格、最新文档、产品对比等联网信息查询。也可手动输入 `/felo-ai 你的问题`。
- `felo-slides`：**PPT 生成**技能。适用于“做一份演示文稿/幻灯片”类请求，也可手动输入 `/felo-slides 你的主题`。

**[搜索技能详细文档 →](./felo-search/)**  
**[PPT 技能详细文档 →](./felo-slides/)**
...
- **[PPT Task API](https://openapi.felo.ai/docs/api-reference/v2/ppt-tasks.html)** - Slides generation reference
